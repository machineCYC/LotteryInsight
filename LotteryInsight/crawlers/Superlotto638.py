...
def parser_win_ball_number(html, is_today):
    # 期別
    draw_term_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_DrawTerm_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_DrawTerm_\d{1,2}">(.*?)</span>'
    )
    draw_terms = re.findall(draw_term_pattern, html)

    # 開獎日期
    ddate_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_Date_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_Date_\d{1,2}">(.*?)</span>'
    )
    ...
    # 開出順序
    no1_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo1_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo1_\d{1,2}">(.*?)</span>'
    )
    on1s = re.findall(no1_pattern, html)

    no2_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo2_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo2_\d{1,2}">(.*?)</span>'
    )
    on2s = re.findall(no2_pattern, html)

    no3_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo3_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo3_\d{1,2}">(.*?)</span>'
    )
    on3s = re.findall(no3_pattern, html)

    no4_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo4_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo4_\d{1,2}">(.*?)</span>'
    )
    on4s = re.findall(no4_pattern, html)

    no5_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo5_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo5_\d{1,2}">(.*?)</span>'
    )
    on5s = re.findall(no5_pattern, html)

    no6_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo6_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo6_\d{1,2}">(.*?)</span>'
    )
    on6s = re.findall(no6_pattern, html)

    sno_pattern = (
        r'<span id="SuperLotto638Control_history1_dlQuery_SNo7_0">(.*?)</span>'
        if is_today
        else r'<span id="SuperLotto638Control_history1_dlQuery_SNo7_\d{1,2}">(.*?)</span>'
    )
    sno = re.findall(sno_pattern, html)

    data = []
    for dt, dd, o1, o2, o3, o4, o5, o6, so7 in zip(
        draw_terms, ddates, on1s, on2s, on3s, on4s, on5s, on6s, sno
    ):
